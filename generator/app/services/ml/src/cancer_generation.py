...
def prepocess_desease(disease, lung_mask, disease_h, disease_w, bpx=5):
    ...
    u = np.random.uniform(low=0.5, high=1, size=(disease_h, disease_w))

    disease = np.maximum(np.minimum(g, disease), disease)
    ...
    disease = disease * u * g * distance * distance
    disease = min_max_normalization(disease)

    disease[disease > 0.5] = 1
    disease = min_max_normalization(disease)

    return disease.astype(np.float32)


def get_cancer(lung_mask, disease_h, disease_w, ):
    disease = ps.generators.blobs(shape=[disease_h, disease_w], porosity=None, blobiness=1)
...
def add_desease_to_part_of_image(ct_part, cancer, lung_mask, ct, body_mask):
    ...
    min_val = (np.min(ct[body_mask]) - global_min_val) / (global_max_val - global_min_val)
    max_val = (np.max(ct[body_mask]) - global_min_val) / (global_max_val - global_min_val)
