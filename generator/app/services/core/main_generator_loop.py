...
class MainGeneratorLoop(AbstractLoopService):
    ...
    def run_step(self):
        ...
        try:
            self._generate_dicoms_with_pathologies(original_dicoms=original_dicoms)
        except Exception as ex:
            logger.error(f'Error during generating DICOM files: {ex}')
            return

        try:
            self._set_research_status(research=research, status=ResearchStatus.GENERATED)
            logger.debug(f'Set status `generated` to research: {research.id} - {research.name}')
        except Exception as ex:
            logger.error(f'Error during setting status `generated`: {ex}')
            return
    ...
    def _get_research_to_generate(self) -> Optional[Research]:
        ...
        try:
            ...
            logger.error(f'Error during getting research: {ex}')
    ...
    def _get_research_original_dicoms(self, research: Research) -> List[Dicom]:
        ...
        try:
            ...
            logger.error(f'Error during getting research DICOM files: {ex}')
    ...
    def _set_research_status(self, research: Research, status: ResearchStatus):
        try:
            ...
            logger.error(f'Error during setting status {status} for research {research.id} - {research.name}: {ex}')

        return research

    def _generate_dicoms_with_pathologies(self, original_dicoms: List[Dicom]):
        for original_dicom in original_dicoms:
            ...
            generated_dicom_bytes = generator.generate_pathologies(original_dicom_bytes=original_dicom_bytes)
            logger.info(f'Generated DICOM file from: {original_dicom.id} - {original_dicom.name}')
            ...
            try:
                cloud_client.upload_by_presigned_url(created_dicom.uploadingUrl, data=generated_dicom_bytes)
